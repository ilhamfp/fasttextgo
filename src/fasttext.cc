...
std::shared_ptr<Loss> FastText::createLoss(std::shared_ptr<Matrix>& output) {
  ...
  switch (lossName) {
    ...
    case loss_name::intentionhs:
      return std::make_shared<IntentionHierarchicalSoftmaxLoss>(
              output, dict_->getLabelCounts(), args_->hfiles);
    default:
      throw std::runtime_error("Unknown loss");
  }
}

std::shared_ptr<Loss> FastText::createOrLoadLoss(std::shared_ptr<Matrix>& output, std::istream& in) {
  if (args_->loss == loss_name::intentionhs) {
    return std::make_shared<IntentionHierarchicalSoftmaxLoss>(output, in);
  } else {
    return createLoss(output);
  }
}
...
void FastText::buildModel(std::istream& in) {
  auto loss = createOrLoadLoss(output_, in);
...
bool FastText::predictLineMaxIntention(
        std::istream& in,
  ...
  std::vector<int32_t> words, labels;
  dict_->getLine(in, words, labels);
  IntentionPredictions linePredictions;
  predictMaxIntention(words, linePredictions);
  for (const auto& p : linePredictions) {
    predictions.push_back(std::make_pair(std::exp(p.first), (p.second)));
  }
