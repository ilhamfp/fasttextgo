    ...
    try {
      ...
      if (args[ai] == "-h") {
        ...
        if (args.at(ai + 1) == "hs") {
          ...
          loss = loss_name::ova;
        } else if (args.at(ai + 1) == "intentionhs") {
          loss = loss_name::intentionhs;
        } else {
      ...
      } else if (args[ai] == "-hfiles") {
        hfiles = std::string(args.at(ai + 1));
...
void Args::printTrainingHelp() {
  std::cerr
      ...
      << "  -loss               loss function {ns, hs, softmax, one-vs-all, intentionhs} ["
      << lossToString(loss) << "]\n"
      << "  -hfiles             hierarchical files for loss=intentionhs\n"
