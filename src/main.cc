...
void predictMaxIntention(const std::vector<std::string>& args) {
  if (args.size() != 4) {
    printPredictUsage();
    exit(EXIT_FAILURE);
  }
  ...
  while (fasttext.predictLineMaxIntention(in, predictions)) {
    printPredictions(predictions, true, false);
  }
  if (ifs.is_open()) {
    ifs.close();
  }
...
int main(int argc, char** argv) {
  ...
  if (command == "skipgram" || command == "cbow" || command == "supervised") {
    ...
    predict(args);
  } else if (command == "predict-max-intention") {
    predictMaxIntention(args);
