...
std::vector<std::pair<std::string, int64_t>> Dictionary::getLabelCounts() {
  std::vector<std::pair<std::string, int64_t>> labelCounts;
  for (int32_t i = nwords_; i < size_; i++) {
    std::pair<std::string, int64_t> pair = std::make_pair(words_[i].word, words_[i].count);
